...
function searchbycolor($id) {
  global $conn;
    $stmt = $conn->prepare("SELECT child.id, child.name, child.fathername, child.mothername, child.weight, child.weight, child.dob, child.weeks, child.kubyimba, child.yarakingiwe, child.ongera, child.sosoma, child.kwamuganga, color.colorname, consellor.name, village.vname FROM child JOIN color ON color.id=child.cid join consellor on consellor.caid=child.caid join village on village.vid=consellor.vid WHERE color.colorname = ? LIMIT 1
    ");
 $stmt->bindParam(1, $id);
 $stmt->execute(); 
...
function allchild() {
  global $conn;
    $stmt = $conn->prepare("SELECT child.id, child.name, 
    child.fathername, child.mothername, 
    child.weight, child.weight, child.dob, child.weeks, child.kubyimba, 
    child.yarakingiwe, child.yarakingiwe, child.ongera, child.sosoma, 
    child.kwamuganga, color.colorname, consellor.name, village.vname 
    FROM child JOIN color ON color.id=child.cid
     join consellor on consellor.caid=child.caid
      join village on village.vid=consellor.vid ORDER BY child.id
    ");
    $stmt->execute();       
    $data = $stmt->fetchAll(PDO::FETCH_OBJ);  
    return $data;
...
function allchildByid($id) {
  global $conn;
    ...
    child.fathername, child.mothername, 
    child.weight, child.weight, child.dob, child.weeks, child.kubyimba, 
    child.yarakingiwe, child.ongera, child.sosoma, 
    child.kwamuganga, color.colorname, consellor.name, village.vname 
    FROM child JOIN color ON color.id=child.cid
     join consellor on consellor.caid=child.caid
...
function findconsellorByid($districtId){
  global $conn;
